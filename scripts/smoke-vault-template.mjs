...
try {
  ...
  assert.match(rootInstall, /repository_url=https:\/\/github\.com\/CoreSaint\/agent-kb/);
  assert.match(rootInstall, /printf 'bootstrap_root=%s\\n' "\$bootstrap_root" >&2/);
  assert.match(rootInstall, /git clone --depth 1 "\$repository_url" "\$bootstrap_root\/agent-kb"/);
  assert.match(rootInstall, /tar -xzf agent-kb-<version>-linux-<arch>\.tar\.gz/);
  assert.match(rootInstall, /\.\/agent-kb-<version>-linux-<arch>\/install\.sh "\$destination"/);
  assert.doesNotMatch(rootInstall, /agent-kb-<version>-<platform>-<arch>/);
  assert.equal(rootInstall.match(/\buname -s\b/g)?.length, 1, "repository install instructions must capture uname exactly once");
  assert.match(rootInstall, /test ! -e "\$destination" && test ! -L "\$destination"/);
